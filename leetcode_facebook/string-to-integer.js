...
// Approach 1: Using Number.parseInt() method already available in javascript
...
// Approach 2: Without Using any built-in methods from javascript
...
var myAtoi = function (s) {
    ...
    const MIN_INT = Number(-(2 ** 31))
    let isNegative = null;
    let isSignDetected = false;
    let result = 0;
    let isDigitDetected = false;

    for(let i=0; i<s.length; i++) {
        const code = s[i].charCodeAt(0);
        if(!isSignDetected && !isDigitDetected && code === 32) { // ' ' - 32
            continue;
        } else if (!isSignDetected && !isDigitDetected && (code === 43 || code === 45)) { // '+' - 43 , '-' - 45
            isNegative = !!(code === 45);
            isSignDetected = true;
